...
takeWhileBetween predicate =
  ...
  where
    ...
    consumeOne = return <$> satisfy predicate

-- | Matches all upper- and lowercase permutations of the specified
-- lower-case string
permuteStr :: String -> Parser String
permuteStr =
  foldr (+>) (pure "") . map (\ x -> char x <|> char (toUpper x))

-- | Disregard the result of a parser, and add a label
disregardWith :: Parser a -> String -> Parser ()
disregardWith p m = p *> pure () <?> m
...
-- | Like 'satisfy' and 'inClass'
satisfyInClass :: String -> Parser Char
satisfyInClass = satisfy . inClass

-- | Like 'satisfy' and 'notInClass'
satisfyNotInClass :: String -> Parser Char
satisfyNotInClass c = satisfy $ not . inClass c
